...
async def get_vehicle_by_set(db: Depends, set_id: set, ):
    """Get user_id by token and device id"""
    sql_id = ""
    for i in set_id:
...
async def total_worker_income(db: Depends, user_id: int,):
    """Получаем актуальные события"""
    data = await db.fetch(f"SELECT SUM(amount) FROM payments WHERE worker_id = $1 AND pay_date != 0;", user_id)
    return data


async def month_worker_income(db: Depends, user_id: int, date: datetime.datetime):
    """Получаем актуальные события"""
    data = await db.fetch(f"SELECT SUM(amount) FROM payments WHERE worker_id = $1 AND pay_date > $2;", user_id,
...
async def worker_close_sessions(db: Depends, user_id: int, date: int = 0):
    """Получаем актуальные события"""
    data = await db.fetch(f"SELECT COUNT(*) FROM service_session "
                          f"WHERE worker_id = $1 "
...
async def read_withdrawal_invoice_payments(db: Depends, wi_id: int):
    ...
    data = await db.fetch(f"SELECT payments.* FROM payments "
                          ...
                          f"WHERE withdrawal.wi_id = $1 ORDER BY payments.worker_id ;", wi_id)
    return data
...
async def read_withdrawal_invoice_for_pdf(db: Depends, wi_id: int):
    ...
    SELECT payments.pay_id, service_session.session_id, service_session.distant, service_session.address, 
    payments.session_work_id, users.name, 
